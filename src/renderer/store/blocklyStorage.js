...
import Blockly, { unbindEvent_ } from 'blockly';
import { LISTS_SPLIT_WITH_DELIMITER } from 'blockly/msg/en';
 ...
 * @private
 */
BlocklyStorage.backupBlocks_ = function (workspace, key) {
 ...
 */
BlocklyStorage.backupOnUnload = function (opt_workspace, key) {
  var workspace = opt_workspace || Blockly.getMainWorkspace();
  window.addEventListener('unload',
    function () { BlocklyStorage.backupBlocks_(workspace, key); }, false);
};
 ...
 * @param {Blockly.WorkspaceSvg=} opt_workspace Workspace.
 */
BlocklyStorage.restoreBlocks = function (opt_workspace, key) {
  if ('localStorage' in window && window.localStorage[key]) {
    var workspace = opt_workspace || Blockly.getMainWorkspace();
    var text = window.localStorage[key]
    if (text === "") {
      workspace.clear()
      return
    }
...
BlocklyStorage.blocksToText = function (workspace) {
